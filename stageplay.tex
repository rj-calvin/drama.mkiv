...
\setupblank[big]
\setupinterlinespace[line=2.8ex, stretch=1, shrink=0]
...
\def\@dostartdialog#1[#2]{%
	\begingroup
	\getparameters[dialog][speaker=\empty, #2]%
	\enablemode[dialogblock]%
	\the\everybeforedialog
	\speaker{\dialogspeaker}%
	\setbox\dialogbox\vtop\bgroup
}
...
\def\@uncontinue{%
	\box\dialogbox\relax
	\setbox\dialogbox\vtop{\unvbox\continuebox\relax}%
}
...
\def\@docontinue{%
	\ifdim\pagegoal < \maxdimen\relax
		\ifdim\pagetotal < \pagegoal\relax
			\ifdim
				\dimexpr4\lineheight + \pagetotal + \ht\dialogbox\relax > \pagegoal
			\relax
				\box\dialogbox\relax
...
\def\continue{%
	\doifmode{dialogblock}{%
		\egroup
		\unless\ifvoid\continuebox
			\@docontinue
		\fi
		\setbox\continuebox\vtop\bgroup
	}%
	\ignorespaces
}
